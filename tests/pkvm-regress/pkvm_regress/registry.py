...
CASES: tuple[TestCase, ...] = (
    TestCase(
        ...
        pass_conditions=("guest 到达 login", "host dmesg 不出现 ptdev MMIO range / deny 日志"),
        fail_conditions=("无 VFIO 也出现 ptdev MMIO range revoked/restored/deny 日志", "guest 启动失败"),
    ...
    TestCase(
        ...
        pass_conditions=("host 新增日志出现 ptdev MMIO range revoked", "guest 能到 login"),
    ...
    TestCase(
        ...
        pass_conditions=("host 新增日志出现 ptdev MMIO range restored", "退出后无 forbidden signature"),
    ...
    TestCase(
        ...
        mode="scripted",
        priority="P0",
        objective="捕获 stale touched DIRECT_BAR range 状态、旧 allowlist、restore 不完整、VFIO group busy 等重复运行问题。",
    ...
    TestCase(
        ...
        priority="P2",
        objective="验证某个 DIRECT_BAR range annotate 失败后，已 touched range 能恢复。",
        trigger="注入 pkvm_host_ept_annotate_mmio_owner() 或 pkvm_pgtable_annotate() 失败。",
        pass_conditions=("已 touched DIRECT_BAR range restore", "touched range state 清零", "owner 回 HOST"),

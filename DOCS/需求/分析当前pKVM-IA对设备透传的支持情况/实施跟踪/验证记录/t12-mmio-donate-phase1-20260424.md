# T12 MMIO donate 第一阶段源码级验证记录 - 2026-04-24
...
- 基线提交：`1cfdacc20432`
- 当前提交：`9f9531b5e36a`
- 本记录只覆盖源码级检查和静态风格检查；未执行 object build、未执行完整 Linux 内核编译、未执行 protected pVM 运行验证。
...
- `OWNER_ID_PTDEV_MMIO`：在 `mem_protect.h` 定义，在 `mem_protect.c` 静态检查，在 `ept.c` deny-remap 分支和 `ptdev.c` BAR revoke 路径使用。
- 旧直接发布路径 `pkvm_update_vm_mmio_allowlist(vm, metadata)`：无输出，说明 `SET_PTDEV_MMIO_METADATA` 不再直接发布 guest allowlist。
...
- 未运行全量内核编译，符合当前项目规则。
- 未运行 object build；如需要，建议由用户确认构建树后再执行以下最小目标：
...
- 未执行 protected pVM + VFIO 运行验证；运行验证仍需覆盖 Case A-E。

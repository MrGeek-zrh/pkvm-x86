...
- 2026-04-16 在包含 T4 第一版实现的 Host 内核上，又运行了一轮推荐矩阵：
...
- 因此当前结论是：
  ...
  - 但复现性仍未收敛，现阶段更适合作为 `T4/T4A` 的验证结果和已归档 `Bug`，而不是已稳定复现的 blocker；
...
- 当前已补 GitHub `Bug` 归档，用于固定唯一签名、原始证据与关闭条件。
- 对应修复动作继续由 `T4` 承载，不把实现方案反向塞回 `Bug` issue。
- `T4` 的实现方向仍应保持不变：
...
- 若后续再次出现相同或邻近签名：
  - 若属于新的 panic / 报错签名，继续单独新建 `Bug`；
  - 不与旧 boot-time 问题混追，也不覆盖当前 `BOOT-014` 的历史语义。
...
- 推荐矩阵（2026-04-16，包含 T4 第一版实现）：
  - [20260416-t4a-recommended-matrix-summary.tsv](raw/20260416-t4a-recommended-matrix-summary.tsv)
  - [20260416-t4a-recommended-matrix-summary.json](raw/20260416-t4a-recommended-matrix-summary.json)
  - 完整逐轮日志目录（本地）：
...
- 当前已经补建 `pkvm-x86#32` 固定症状，并由 `pkvm-x86#8` 承载修复动作；但因为还没有稳定复现，所以仍不建议只凭这一条正例就宣称问题已经完全收敛，更合理的做法是继续把 `T4A` 跑稳。
